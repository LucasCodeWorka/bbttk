...
import { syncEmProducao } from '../services/totvs.service.js';
...
async function sincronizarEmProducao(dryRun: boolean, detalhado: boolean): Promise<SyncResumo> {
  ...
  console.log(`[em-producao] dry-run: ${linhas.length} linhas agregadas seriam geradas (nenhuma escrita no banco).`);
  if (detalhado) {
...
async function main(): Promise<void> {
  ...
  if (!dryRun) {
    // A sincronizacao real (fetch + DELETE/INSERT em ops_em_producao, schema completo
    // de 13 colunas) mora em totvs.service.ts:syncEmProducao() - reusada aqui em vez de
    // duplicada pra nao divergir de novo do schema real da tabela.
    const resumoReal = await syncEmProducao();
    console.log(`[em-producao] fim ${new Date().toISOString()}`);
    console.log(`[em-producao] resumo: ${resumoReal.ordens} ordens vistas, ${resumoReal.linhas} linhas gravadas em ops_em_producao.`);
    return;
  }

  console.log('[em-producao] modo dry-run ativo.');

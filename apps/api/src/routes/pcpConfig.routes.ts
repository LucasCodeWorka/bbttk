...
router.put('/pcp-config', async (req: Request, res: Response) => {
  try {
    const { relatorio, giroDias, coberturaMeses, atacadoCoberturaBase, custoCode, pdvVarejoCode, pdvAtacadoCode, precoCustoBranchCode } = req.body;
    if (!relatorio) {
    ...
    const config = await pcpConfigService.updateConfig(
      { relatorio, giroDias, coberturaMeses, atacadoCoberturaBase, custoCode, pdvVarejoCode, pdvAtacadoCode, precoCustoBranchCode },
      req.user?.userId
    );
    res.json({ config });
  } catch (error) {
...
router.put('/pcp-config/curva-abc', async (req: Request, res: Response) => {
  try {
    const { relatorio, giroDias, metaCurvaAUnidades, curvaDPercent, curvaCPercent } = req.body;
    if (!relatorio) {
      res.status(400).json({ error: 'relatorio e obrigatorio' });
      return;
    }
    ...
    const config = await pcpConfigService.updateCurvaAbcConfig(
      { relatorio, giroDias, metaCurvaAUnidades, curvaDPercent, curvaCPercent },
      req.user?.userId
    );
    res.json({ config });
  } catch (error) {

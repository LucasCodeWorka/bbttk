...
import { pcpConfigApi, PcpCoberturaIdealItem, PcpCodigosDisponiveis } from '@/lib/api';
...
export default function RelatorioBaseConfigPage() {
  ...
  const carregarDados = useCallback(async () => {
    ...
    setIsLoading(true);
    try {
      const [configRes, coberturaRes, codigosRes] = await Promise.all([
        pcpConfigApi.getConfig(token, RELATORIO),
        pcpConfigApi.getCoberturaIdeal(token, RELATORIO),
        pcpConfigApi.getCodigosDisponiveis(token),
      ]);
    ...
    <div className="space-y-6">
      ...
      <Card>
        ...
        <p className="text-sm text-gray-500 -mt-2 mb-4">
          ...
          coluna do Relatório Base. {codigos.custos.length === 0 && 'Sincronize pelo menos uma vez antes de escolher.'}
        </p>
        <div className="flex flex-wrap gap-4 items-end">
          <Select
            label="Custo"
